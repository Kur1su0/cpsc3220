...
def main():
  ...
  if not any(
    os.path.exists(os.path.join(root, 'config'))
  ...
  if opt.user is None:
    if config.has_option(opt.remote, 'username'):
      ...
      config.has_option(opt.remote, 'use_system_username') and
      config.getboolean(opt.remote, 'use_system_username')
    ):
      opt.user = DEFAULT_USER
    elif config.has_option(CONFIG_SECTION, 'username'):
      opt.user = config.get(CONFIG_SECTION, 'username')
    elif (
      config.has_option(CONFIG_SECTION, 'use_system_username') and
      config.getboolean(CONFIG_SECTION, 'use_system_username')
    ):
      opt.user = DEFAULT_USER
    else:
      opt.user = raw_input('Username (%s): ' % DEFAULT_USER)
...
def _get_curl(config, remote, command):
  """ Return a boilerplate pycurl.Curl object.
  """
  headers = [
  ...
  curl.setopt(pycurl.URL,
              config.get(remote, 'url') + '?command=' + command)
...
def _post(config, remote, command, postargs, cabundle):
  ...
  response = StringIO()
  headers = Headers()
  curl = _get_curl(config, remote, command)
  ...
  if content_type is None:
    ...
    raise HandinError("Content-Type (%s) not understood (status: %d)" % (
        content_type,
        status,
      )
    )
...
def _get_menu_choices(menu):
  ...
  if menu['type'] == 'selection':
    ...
    choices = [_get_menu_choices(submenu) for submenu in
               menu['choices'].values()]
...
def _print_menu_choices(menu, key, choices, level=0):
  ...
  if menu['type'] == 'selection':
    ...
    print "%s%s" % (indent, menu.get('display_name', key))
    for k in menu['choices'].keys():
...
def _interactive_select_bucket(config, remote, username, password, cabundle):
  ...
  try:
    ...
    if status >= 400:
      _warn('%s [%d]' % (
          data.get('message', '(unknown error)'), status
        ),
        extra={'tag': remote},
      )
      _fail("Submission failed.")
...
def _interactive_static_menu(menu):
  ...
  if not choices:
    return None
  choices = dict((str(k), v) for k, v in choices.items())
  rchoices = dict((v, k) for k, v in choices.items())
  _print_menu_choices(menu, 'Menu', rchoices)
  choice = None
  while not choice:
    choice = raw_input("Select bucket: ")
...
class CursesMenu(object):
  def __init__(self, menu, stdscreen, pretitle='', auto_select_depth=2):
    ...
    if pretitle:
      ...
      self.title = name
    while (
      auto_select_depth > 0 and
      menu['type'] == 'menu' and
      len(menu['choices']) == 1
    ):
      auto_select_depth -= 1
      k, menu = menu['choices'].items()[0]
    ...
    while True:
      ...
      if key in [curses.KEY_ENTER, ord('\n')]:
        ...
        if item == 'CANCEL':
          ...
          submenu = CursesMenu(item, self.screen, self.title,
                               auto_select_depth=0)
...
def _interactive_curses_menu(menu):
  result = curses.wrapper(
    lambda stdscreen: _curses_menu_entry_point(menu, stdscreen)
  )
  if result == 'CANCEL':
...
def _interactive_configure(global_config):
  if (
    os.path.exists(USER_CONFIG_ROOT) and
    not os.path.isdir(USER_CONFIG_ROOT)
  ):
  ...
  if not global_config.has_option(CONFIG_SECTION, 'remote'):
    ...
    while (
      not remote or
      remote == CONFIG_SECTION or
      remote.lower() == 'default'  # Python throws error for this
    ):
      if remote is not None:
        _warn("Invalid remote name.")
      remote = (raw_input("Default remote name (%s): " % DEFAULT_REMOTE) or
                DEFAULT_REMOTE)
    while not url:
      url = raw_input("Enter submit URL: ")
  ...
  use_system_username = (
    global_config.has_option(CONFIG_SECTION, 'use_system_username') and
    global_config.getboolean(CONFIG_SECTION, 'use_system_username')
  )
  if not (has_username or use_system_username):
    username = raw_input("Default username (leave blank for none): ")
    if username:
...
def _get_parser():
  ...
  parser.add_option('-r', '--remote', default=None,
                    help='Remote name to submit to')
  parser.add_option('-u', '--user', default=None,
                    help='Username to log in with')
  parser.add_option('-b', '--bucket', default=None,
                    help='Bucket identifier')
  parser.add_option('-q', '--quiet', default=0, action='count',
                    help='Show less output')
  parser.add_option('-v', '--verbose', default=0, action='count',
                    help='Show more output')
  parser.add_option('-C', '--cabundle', default=None,
  ...
  parser.add_option('--remotes', action='store_true',
                    help='List configured remote names.')
  return parser
